import {
  LineChart, ShieldCheck, Shield, Star, Target, Trophy, Users,
  Dumbbell, Megaphone, Wand2, FileCheck, BookOpen, Camera,
  Building2, Brain, CheckCircle2, ArrowRight,
} from "lucide-react";
  ...
  return (
    <div className="relative size-[84px] shrink-0">
      <svg className="size-full -rotate-90" viewBox="0 0 84 84">
        <circle cx="42" cy="42" r={r} fill="none" stroke="currentColor" strokeWidth="4" className="text-muted" />
        <circle cx="42" cy="42" r={r} fill="none" stroke="currentColor" strokeWidth="4"
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="text-[9px] font-medium text-muted-foreground uppercase tracking-wide mt-0.5">OVR</span>
      </div>
    </div>
  );
}
...
const FEATURES = [
  { Icon: ShieldCheck, title: "Digital Player Passport",    text: "A verified, shareable profile with a unique QR code. Scouts and coaches can see ratings, attributes, and milestones at a glance." },
  { Icon: Wand2,       title: "AI Session Generator",       text: "Builds LTPD-phased drills with 4-Corner coaching cues, a FIFA session structure, and SAFA NDP competency alignment." },
  { Icon: Brain,       title: "AI Match and Player Reports", text: "Post-match reports, individual insights, and parent letters, each grounded in FIFA technical methodology and SAFA standards." },
  { Icon: LineChart,   title: "Academy Analytics",          text: "SAFA-benchmarked dashboards showing position spread, rating trends, document compliance, training attendance, and top performers." },
  { Icon: Star,        title: "Performance Ratings",        text: "Coaches rate players out of 5 after every match or session. Notes, history, and trend charts build a long-term record." },
  { Icon: Dumbbell,    title: "Training Management",        text: "Plan sessions with drill lists, session types, attendance tracking, and media uploads tagged to individual players." },
  { Icon: FileCheck,   title: "SAFA Document Compliance",   text: "Track all 6 registration documents per player, per season: registration agreement, POPIA consent, medical consent, and more." },
  { Icon: BookOpen,    title: "Development Milestones",     text: "Five development categories (Technical, Tactical, Physical, Mental, Leadership) with milestone tracking each season." },
  { Icon: Trophy,      title: "Match Centre",               text: "Fixtures, results, squad appearances, match attendance, and win/draw/loss records in one place for every team." },
  { Icon: Megaphone,   title: "Announcements",              text: "Coaches post updates straight to the squad. Players and parents stay informed, and coaches can see who has read each message." },
  { Icon: Camera,      title: "Media Gallery",              text: "Upload photos and videos from training or matches, tag players, and build a visual record of every session." },
  { Icon: Target,      title: "Multi-Role Platform",        text: "Admin, Coach, Player, and Parent each get their own dedicated dashboard. One account, one academy, everyone connected." },
];
...
const FRAMEWORKS = ["FIFA LTPD", "SAFA NDP", "CAF Pathway", "4-Corner Model", "SAFA Registration", "UEFA Best Practice"];
    ...
    <div className="flex min-h-dvh flex-col">
        ...
        <div className="mx-auto flex h-16 w-full max-w-6xl items-center justify-between px-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button variant="ghost" size="sm" asChild>
              <a href={`${APP_URL}/auth/login`} className="hidden sm:inline-flex">Sign in</a>
            </Button>
      ...
      <main className="flex-1">
        ...
        <section className="mx-auto grid w-full max-w-6xl items-center gap-10 px-4 py-14 sm:px-6 lg:grid-cols-2 lg:py-24">
          <div>
            <Badge variant="brand" className="mb-4">#WeBuildChampions</Badge>
            ...
            <p className="mt-5 max-w-lg text-base text-muted-foreground sm:text-lg leading-relaxed">
              ...
              Academies get analytics that lines up with what SAFA and FIFA actually expect.
            </p>
          ...
          <Card className="mx-auto w-full max-w-sm overflow-hidden shadow-xl">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="space-y-2.5 pt-1">
                {ATTRIBUTES.map((a) => <StatBar key={a.label} label={a.label} value={a.value} />)}
              </div>
        ...
        <div className="border-y border-border bg-muted/40">
          <div className="mx-auto flex w-full max-w-6xl flex-wrap items-center justify-center gap-x-8 gap-y-3 px-4 py-4 sm:px-6">
            ...
            {FRAMEWORKS.map((f) => <span key={f} className="text-sm font-medium text-foreground/70">{f}</span>)}
          </div>
        </div>
        ...
        <section id="roles" className="mx-auto w-full max-w-6xl px-4 py-16 sm:px-6">
          <div className="mb-10">
            ...
            <p className="mt-2 text-muted-foreground max-w-xl">Every person in the academy sees what matters to them, without the clutter.</p>
          </div>
            ...
            <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
              <div>
                ...
                <p className="mt-4 text-muted-foreground leading-relaxed">
                  Each report is built around the FIFA Long-Term Player Development framework,
                  the 4-Corner Model, and SAFA&apos;s National Development Programme. So coaches
                  and parents get feedback that&apos;s grounded in how youth development actually
                  works, not just numbers on a screen.
                </p>
              </div>
        ...
        <section id="features" className="mx-auto w-full max-w-6xl px-4 py-16 sm:px-6">
          <div className="mb-10">
            ...
            <p className="mt-2 text-muted-foreground max-w-xl">
              From the first registration document to match day, Growfit FA handles the admin so coaches can focus on the football.
            </p>
          </div>
        ...
        <section className="border-t border-border bg-muted/40">
          <div className="mx-auto flex w-full max-w-6xl flex-col items-center gap-5 px-4 py-16 text-center sm:px-6">
            ...
            <h2 className="text-3xl font-bold tracking-tight max-w-xl">Get your academy up and running today.</h2>
            ...
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <Button size="lg" variant="outline" asChild>
                <a href={`${APP_URL}/auth/login`}>Sign in to your academy</a>
              </Button>
            </div>
          </div>
        </section>
            ...
            <div className="flex flex-col items-start gap-3 sm:items-end">
              <div className="flex flex-wrap gap-2">
                {["FIFA LTPD", "SAFA NDP", "CAF Pathway", "4-Corner Model"].map((f) => (
                  <span key={f} className="rounded-full border border-border px-2.5 py-0.5 text-[10px] font-medium text-muted-foreground">{f}</span>
                ))}
              </div>
              <p className="text-sm text-muted-foreground">
                &copy; {new Date().getFullYear()} Growfit FA
              </p>
            </div>
          </div>
        </div>
